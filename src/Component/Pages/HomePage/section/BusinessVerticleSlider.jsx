...
const BusinessVerticleSlider = () => {
  ...
  useEffect(() => {
    ...
    gsap.set(panels[0], { autoAlpha: 1, display: "block", y: 0, zIndex: 10 });
    gsap.set(panels.slice(1), { opacity: 0, y: 50, zIndex: 0 });
    const tl = gsap.timeline({
      scrollTrigger: {
        trigger: scroller,
        scroller: "body",
        start: "top 10%",
        end: `+=${slides.length * 1000}`, // <-- Give each slide enough scroll space
        scrub: true,
        pin: true,
        // markers: true,
        snap: {
        ...
        onUpdate: (self) => {
          const slideIndex =
            Math.round(self.progress * (slides.length - 1)) + 1;
    ...
    panels.forEach((panel, i) => {
      if (i > 0) {
        tl.to(panels[i - 1], {
          ...
          .set(panels[i - 1], { display: "none" }) // hide previous
          .set(panels[i], { display: "block" }) // show current
          .to(
            panels[i],
            {
              autoAlpha: 1,
  ...
  return (
    // <div className="" style={{ height: "100vh", overflow: "hidden" ,backgroundColor: "lightgray"}}>
    <section className="scroller max-w-screen-lg lg:mx-auto mx-4 mt-[120px] ">
      <p className="max-[360px]:text-[22px] text-[24px] lg:text-[34px] font-medium text-[#FFF] text-center lg:mb-[50px] mb-[30px]">
        Grow your{" "}
        <span className="business-gradient max-[360px]:text-[28px] text-[32px] lg:text-[46px] font-semibold">
          business
        </span>{" "}
        with simplified billing solutions
      </p>
      ...
      <div className="relative w-full h-[500px] overflow-hidden">
        ...
        {slides.map((slide, idx) => (
          <div
            className="panel flex items-center justify-center business-slider-parent px-3 pt-[20px] pb-[20px] lg:p-6"
            key={slide.id}
          >
            <div
              ref={containerRef}
            ...
            >
              <div
                className=" w-full h-full flex flex-col-reverse md:flex-row items-center justify-center gap-[50px] pr-[14px] "
                style={{
                  height: slideHeight,
              ...
              >
                {/* 🎯 Image Container */}
                <div className="slide-image w-full md:w-1/2 flex h-[240px] lg:h-full justify-center items-center business-slider-image-sec">
                ...
                {/* 📝 Content Container */}
                <div className="slide-text w-full md:w-1/2 h-[50%] lg:h-full flex items-center">
                  <div>
