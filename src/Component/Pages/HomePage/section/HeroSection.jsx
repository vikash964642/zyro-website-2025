...
export default function HeroSection() {
  ...
  return (
    <div className="relative">
      ...
      <button
        onClick={handlePrev}
        className="absolute z-50 left-[100px] min-[1500px]:left-[200px] top-[190px] text-white bg-gray-700 rounded-full w-[40px] h-[40px] hover:bg-gray-800 lg:flex justify-center items-center hidden"
      >
        &lt;
      </button>
      <button
        onClick={handleNext}
        className="absolute z-50 right-[100px] min-[1500px]:right-[200px] top-[190px] text-white  bg-gray-700 rounded-full w-[40px] h-[40px] hover:bg-gray-800 lg:flex justify-center items-center hidden"
      >
        &gt;
      </button>
  ...
  >
    {slides.map((slide, index) => (
      <div key={index} className="min-w-full flex flex-col-reverse lg:flex-row items-center gap-5">
        {/* Left Content */}
          ...
          <h1 className="md:text-[27px] xl:text-[42px] hidden lg:block text-white text-[22px] font-medium lg:leading-[37px] xl:leading-[55px] leading-[24px] lg:mt-[10px] max-[400px]:text-[18px]">
            {slide.heading.split(" ").map((word, idx) =>
              ["Connected", "Banking", "Solutions"].includes(word) ? (
                <span key={idx} className="bg-gradientText bg-clip-text text-transparent font-semibold">
                  {word}{" "}
                </span>
              ) : (
                word + " "
        ...
        <div className="p-[25px] md:p-[0px] lg:w-[45%] w-full">
         ...
         <h1 className="md:text-[27px] xl:text-[42px] lg:hidden text-white text-[22px] text-center font-medium lg:leading-[37px] xl:leading-[55px] leading-[24px] lg:mt-[10px] max-[400px]:text-[18px]">
            {slide.heading.split(" ").map((word, idx) =>
              ["Connected", "Banking", "Solutions"].includes(word) ? (
                <span key={idx} className="bg-gradientText bg-clip-text text-transparent font-semibold text-[32px] leading-[38px]">
                  {word}{" "}
                </span>
              ) : (
                word + " "
